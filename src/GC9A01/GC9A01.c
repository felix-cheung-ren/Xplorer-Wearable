...
static void GC9A01_Select(void)   { R_GPIO_W_PinWrite(&g_gpio_w_ctrl, GC9A01_CS_PIN, BSP_IO_LEVEL_LOW); }
static void GC9A01_Unselect(void){ R_GPIO_W_PinWrite(&g_gpio_w_ctrl, GC9A01_CS_PIN, BSP_IO_LEVEL_HIGH);   }
static void GC9A01_DC_Command(void){ R_GPIO_W_PinWrite(&g_gpio_w_ctrl, GC9A01_DC_PIN, BSP_IO_LEVEL_LOW); }
static void GC9A01_DC_Data(void)   { R_GPIO_W_PinWrite(&g_gpio_w_ctrl, GC9A01_DC_PIN, BSP_IO_LEVEL_HIGH);   }

// === DELAY ===
static void GC9A01_Delay(uint32_t ms) { R_BSP_SoftwareDelay(ms, BSP_DELAY_UNITS_MILLISECONDS); }

// === SPI ===
static void GC9A01_SPI_Transmit(uint8_t *data, uint32_t size)
{
	tx_busy = 1;
	R_SPI_W_Write(GC9A01_SPI_CTRL, data, size, SPI_BIT_WIDTH_8_BITS);
	while (tx_busy);
}
...
{
	R_GPIO_W_PinWrite(&g_gpio_w_ctrl, GC9A01_RST_PIN, BSP_IO_LEVEL_LOW);
	GC9A01_Delay(20);
	R_GPIO_W_PinWrite(&g_gpio_w_ctrl, GC9A01_RST_PIN, BSP_IO_LEVEL_HIGH);
	GC9A01_Delay(120);
}
...
void GC9A01_WriteDataBuffer(uint8_t *data, uint32_t size)
{
	...
	GC9A01_DC_Data();
#if USE_DMA
	GC9A01_SPI_Transmit(data, size);
#else
	APP_PRINT("Non-DMA unsupported");
#endif
}
...
void GC9A01_Flush(const void *color_map, int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
	...
	tx_busy = 1;
	flush_pending = 1;
	R_SPI_W_Write(GC9A01_SPI_CTRL, (uint8_t *)color_map, size, SPI_BIT_WIDTH_8_BITS);
#else
	APP_PRINT("Non-DMA unsupported");
#endif
}
...
__attribute__((weak)) void GC9A01_FlushReady(void)
{
