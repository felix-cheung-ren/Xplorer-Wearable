 ...
 **********************************************************************************************************************/
#include "bsp_api.h"
#include "../../../../mcu/all/bsp_preferred_reg_values.h"
...
__STATIC_FORCEINLINE void bsp_preferred_settings_early_init(void);
...
void SystemInit (void)
{
    ...
    *p_main_stack_top = BSP_TZ_STACK_SEAL_VALUE;
    ...
    bsp_preferred_settings_early_init();
...
__STATIC_FORCEINLINE void bsp_debugger_enable (void)
{
    ...
    CRG_TOP->PIN_MUX_CTRL_REG_b.SWD_PIN_MUX = 0;
 #endif
}
...
/* Set preferred settings for register values in each power domain */
    for (uint8_t pd = 0; pd < BSP_PD_ID_COUNT; pd++)
    {
        bsp_set_preferred_reg_values(pd);
    }
  #endif
}
...
static void SystemRuntimeInit (const uint32_t external)
{
 ...
 #if defined(__ICCARM__) || BSP_MCU_GROUP_RA6W3
