...
bool bsp_clock_xtalk_is_enabled (void)
{
    ...
    return !!(RTC->CLK_XTAL32K_REG_b.XTAL_BAT_EN);
 #else
 ...
 #if !BSP_MCU_GROUP_RA6W1              // TODO remove this
...
{
#if BSP_MCU_GROUP_RA6W1
    SystemCoreClock = R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_SYS_CLK);
#else
...
void R_BSP_LPClockUpdate (void)
{
    ...
    lp_clk_is_t lp_clk      = bsp_prv_lpclk_get();
    if (lp_clk == LP_CLK_IS_XTAL32K)
    {
        clock_index = BSP_CLOCKS_SOURCE_CLOCK_XTALK;
    }
    else if (lp_clk == LP_CLK_IS_RCX)
    {
        clock_index = BSP_CLOCKS_SOURCE_CLOCK_RCX;
    }
    g_bsp_lp_clock = g_clock_freq[clock_index];
 #endif
}
...
{
    static const uint32_t mask = CRG_TOP_CLK_RCLP_REG_RCLP_LOW_SPEED_FORCE_Msk |
                                 CRG_TOP_CLK_RCLP_REG_RCLP_HIGH_SPEED_FORCE_Msk;
    uint32_t value;
    ...
    if (BSP_CLOCK_RCLP_SPEED_512K == speed)
    {
        ...
        freq  = BSP_RCLP_HIGH_SPEED_FREQ_HZ;
    }
...
#else
    uint32_t clock_index = SYS_CLK_IS_NONE;
    if (FSP_PRIV_CLOCK_XTALM == clock)
    {
        clock_index = SYS_CLK_IS_XTAL40M;
    }
    else if (FSP_PRIV_CLOCK_PLL == clock)
    {
        clock_index = SYS_CLK_IS_PLL240M;
    }
#endif
    FSP_CRITICAL_SECTION_DEFINE;
    FSP_CRITICAL_SECTION_ENTER;
...
void bsp_clock_init (void)
{
   ...
   #if BSP_FEATURE_CGC_RCX_STABILIZATION_MAX_US
    R_BSP_SoftwareDelay(BSP_FEATURE_CGC_RCX_STABILIZATION_MAX_US, BSP_DELAY_UNITS_MICROSECONDS);
   #endif
  #endif
...
void bsp_clock_lpclk_set (fsp_priv_source_clock_t clock)
{
    ...
    if (FSP_PRIV_CLOCK_RCX == clock)
    {
        bsp_prv_lpclk_select(LP_CLK_IS_RCX);
    }
    else if (FSP_PRIV_CLOCK_XTALK == clock)
    {
        bsp_prv_lpclk_select(LP_CLK_IS_XTAL32K);
    }
    FSP_CRITICAL_SECTION_EXIT;
 #endif
}
