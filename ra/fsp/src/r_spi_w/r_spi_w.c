...
fsp_err_t R_SPI_W_Open (spi_ctrl_t * p_api_ctrl, spi_cfg_t const * const p_cfg)
{
    ...
    /* Initialize transfer state fields to clean state. The count field is especially critical:
     * it acts as a software busy gate to prevent overlapping transfers, so it must start at 0.
     * IAR compiler may not zero-initialize stack variables, causing uninitialized garbage values. */
    p_ctrl->count     = 0U;
    p_ctrl->tx_count  = 0U;
    p_ctrl->rx_count  = 0U;
    p_ctrl->p_tx_data = NULL;
    p_ctrl->p_rx_data = NULL;
...
static void r_spi_w_transfer_end (spi_w_instance_ctrl_t * p_ctrl)
{
    ...
    /* Mask all SPI IRQ sources before final cleanup to avoid stale events between
    ...
    /* If SPI is disabled, flush FIFO state now so the next transfer starts from
     * a deterministic hardware state. */
    if (0U == p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_EN)
    {
        p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_FIFO_RESET = 1U;
        (void) p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_FIFO_RESET;
        p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_FIFO_RESET = 0U;
        (void) p_ctrl->p_regs->SPI_FIFO_STATUS_REG;
    }
...
static fsp_err_t r_spi_w_start_transfer (spi_w_instance_ctrl_t * p_ctrl)
{
    ...
    /* Clear any stale IRQ state from the previous transfer before arming a new one. */
    R_BSP_IrqStatusClear(p_extend->gen_irq);

    /* Ensure SPI is disabled before attempting FIFO reset. Back-to-back transfers from previous
     * test may leave SPI_EN in transition state across clock domains. Readback SPI_EN to create
     * synchronization barrier. */
    p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_EN = 0U;
    (void) p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_EN;         /* Readback barrier to ensure SPI_EN=0 propagates */

    /* Clear the FIFO. Perform multiple readbacks to ensure reset takes effect in the SPI peripheral
     * clock domain and that FIFO is truly emptied before starting new transfer. */
    p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_FIFO_RESET = 1U;
    (void) p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_FIFO_RESET; /* Readback barrier */
    p_ctrl->p_regs->SPI_CTRL_REG_b.SPI_FIFO_RESET = 0U;
    (void) p_ctrl->p_regs->SPI_FIFO_STATUS_REG;           /* Readback FIFO status to confirm reset took effect */
...
static void r_spi_w_receive (spi_w_instance_ctrl_t * p_ctrl)
{
    ...
    /* Guard against spurious/late RX IRQ entries (e.g. pending/latched NVIC state or
     * full-duplex RX bytes generated by a write-only transfer). All expected frames have
     * already been consumed, so exit before touching the FIFO, rx buffer pointer or counters. */
    ...
    /* Retune RX_TL so the final partial FIFO chunk still generates an interrupt. */
    remaining_words = p_ctrl->count - p_ctrl->rx_count;
    remaining_bytes = remaining_words * p_ctrl->wordsize;

    if (remaining_words &&
        (remaining_bytes < (uint32_t) (p_ctrl->p_regs->SPI_FIFO_CONFIG_REG_b.SPI_RX_TL + 1U)))
    {
        p_ctrl->p_regs->SPI_FIFO_CONFIG_REG_b.SPI_RX_TL = (uint8_t) (remaining_bytes - 1U);
    }
}
...
static void r_spi_w_transmit (spi_w_instance_ctrl_t * p_ctrl)
{
    ...
    if (0U == p_ctrl->p_cfg->channel)
    {
        fifo_lvl_rem = ((uint32_t) SPI_W_TX_FIFO_MAX_SIZE_BYTE_SPI1 -
                        p_ctrl->p_regs->SPI_FIFO_STATUS_REG_b.SPI_TX_FIFO_LEVEL) /
                       p_ctrl->wordsize;
    }
    else
    {
        fifo_lvl_rem = ((uint32_t) SPI_W_TX_FIFO_MAX_SIZE_BYTE -
                        p_ctrl->p_regs->SPI_FIFO_STATUS_REG_b.SPI_TX_FIFO_LEVEL) /
                       p_ctrl->wordsize;
    }

#else
    fifo_lvl_rem = ((uint32_t) SPI_W_RX_FIFO_MAX_SIZE_BYTE -
                    p_ctrl->p_regs->SPI_FIFO_STATUS_REG_b.SPI_TX_FIFO_LEVEL) /
                   p_ctrl->wordsize;
#endif

    remaining_words = p_ctrl->count - p_ctrl->tx_count;
    words_send      = (remaining_words < fifo_lvl_rem) ? remaining_words : fifo_lvl_rem;

    for (uint8_t i = 0U; i < words_send; i++)
    {
        if (NULL != p_ctrl->p_tx_data)
        {
            if (SPI_BIT_WIDTH_16_BITS < p_ctrl->bit_width)      /* Bit Widths > 16 bits */
