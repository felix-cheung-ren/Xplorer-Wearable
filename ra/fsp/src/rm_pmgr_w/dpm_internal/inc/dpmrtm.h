...
#ifndef __DPMRTM_PATCH_H__
#define __DPMRTM_PATCH_H__
...
#define DPM_ACCEPT_IPV6_MC_CNT  4
...
struct bcn_offset {
	...
	long long      r0;          /* BCN 0 */
	long long      r1;          /* BCN 1 */
	long           min_optimal_tbtt; // NETGEAR R6400; Invalid
	long           min_d;
	long           min_to;
	long           min_d_to;
...
struct dpm_status {
	...
	unsigned int   pm_status: 1;
	unsigned int   reserved1: 1;
	unsigned int   rssi_status: 1;
	unsigned int   reserved2: 16;
	/// The flag indicating whether Null frame TX failed in the previous PTIM.
	unsigned int   txretry_ka_en: 1;
	/// The flag indicating whether SetPS TX failed in the previous PTIM.
	unsigned int   txretry_setps_en: 1;
	/// BUFP has detected a beacon with a UC signal.
	unsigned int   bufp_uc_detect: 1;
	/// If the probe fails and no beacons have been received, retry to receive
	/// an ARP reply frame.
	unsigned int   bufp_probe_pending: 1;
	unsigned int   bufp_arp_status: 1;
	unsigned int   bufp_probe_status: 1;
	unsigned int   bufp_status: 3;
	unsigned int   uc_retry: 1;
	unsigned int   ps_type: 2;            ///< smart ps-poll type
	unsigned int   determined_ps_type: 1; ///< determining ps-poll type
	...
	unsigned short rx_on_us_round;
	unsigned char  reserved_0;
	unsigned char  uc_retry_cnt;

	unsigned long  cs_on_us;
	unsigned long  rx_on_us;

	/// It is time to finish receiving UC.
	unsigned long  uc_end_us;

	/// UC RX Status
	unsigned long  uchk_status;
	/// The last sleep time in milliseconds for DBG
	unsigned long  fullboot_sleep_us;
	unsigned long  fullboot_max_sleep_us;
	unsigned long  fullboot_min_sleep_us;

	/// The current bufp count in bufp.period_ty unit.
	unsigned short bufp_chk_cnt;

	/// The bufp count after buffering time probe completion.
	unsigned short bufp_done_cnt;

	/// The maximum bufp count
	unsigned short bufp_max_cnt;

	/// The minimum bufp count
	unsigned short bufp_min_cnt;

	/// The bufp retry count
	unsigned short bufp_retry_cnt;

	/// BUFP probe success count
	unsigned char  bufp_probe_cnt;

	/// BUFP arp reply frame reception count
	unsigned char  bufp_arp_cnt;

	/// BUFP transmitted ARPREQ frame count status
	unsigned short bufp_arpreq_retry_cnt;

	/// BUFP probe count status
	unsigned short bufp_probe_retry_cnt;

	/// The primary channel of HT Information.
	unsigned char  primary_ch;
	unsigned char  reserved_1;
	/// AP Capabilities Check
	unsigned short ap_cap;

	/// TxRetry type
	// unsigned long  txretry_type;
	...
	struct txcnt_ty {
		...
		union {
			struct {
				unsigned char setps_a[4];
			};
			unsigned long setps;
		};
	} txcnt;

	unsigned char  dcdc_st_ctrl;
	unsigned char  reserved_3;
	unsigned short tcp_response_time;

	unsigned long  tcp_rx_crc;
	unsigned short tcp_rx_len;
	/// Time up to the first TCP TX from full-boot
	unsigned short tcp_tx_time;
...
struct timp_feature_ty {
	union {
		...
		struct {
			...
			// Enable TIMP_0 Auto Wakeup
			unsigned char chk_wakeup_0: 1;
			// Enable TIMP_1 Auto Wakeup
			unsigned char chk_wakeup_1: 1;
...
struct uc_feature_ty {
	union {
		...
		struct {
			...
			unsigned char morechk: 1;
			unsigned char rx_timeout_control: 1;
			unsigned char chk_wakeup_en: 1;
			unsigned char retry_en: 1;
	...
	// unsigned char  postprep_tu;
	unsigned short reserved2;
	unsigned long  chk_period;
	long           reserved3;
	long long      reserved4;
};
...
struct ka_feature_ty {
    ...
    /// period_ms is the period of KA in milli-second.
    /// PTIM don't use period_ms.
    /// It is used to calculate the KA period in Fullboot.
...
struct arp_feature_ty {
    ...
    /// The period of auto ARP in KA units
	unsigned short period_ka;
...
struct udph_feature_ty {
    ...
    /// A source port of UDPH
	unsigned short sport;
    /// A destination port of UDPH
	unsigned short dport;
    ///  A pointer to UDPH target MAC address
	unsigned short target_mac[4];
    /// A destination IP address of UDPH
	unsigned long  dip;

    /// UDPH payload type
	unsigned int   type: 8;
    /// The length of UDPH payload
	unsigned int   payload_len: 24;
    /// A pointer to UDPH user data address
	unsigned long  user_data;
    /// The period of UDPH in KA units
	unsigned short period_ka;
    unsigned short reserved0;
	long           reserved1;
	long long      reserved2;
...
// Brown out/Black out features
struct bor_feature_ty {
	unsigned int   en: 5;         // Brown out state enable
	unsigned int   reserved0: 3;
	unsigned int   reset_en: 5;   // RTC Reset by brown out state
...
struct tcpka_feature_ty {
    ...
    /// The base address of user data
    unsigned long  user_data;

    /// Check new TCP port because AP may receive a TCP frame with RST flag
    /// from a server. An application must set chkport for checking the new TCP
    /// port. chkport_en is automatically reset by Fullboot.
    unsigned int chkport_en: 1;
    /// The period_ty to checking new port
    unsigned int chkport_period_ty: 2;
    /// The period_ty to checking new port in KA units
    unsigned int chkport_period_ty_ka: 1;
    /// The period to checking new port
    unsigned int chkport_period: 12;
    /// The destination port for chkport
    unsigned short chkport_dport: 16;

    /// The period of TCPKA in KA units
    unsigned short period_ka;
    unsigned short reserved2;
};

struct fboot_feature_ty {
	unsigned int period0_100ms: 22;
	unsigned int period0_drift: 10;

	unsigned int period1_100ms: 22;
	unsigned int period1_drift: 10;
};
...
struct deauth_feature_ty {
	...
	unsigned char  reserved1;
    /// The period of DeauthChk in KA units
	unsigned short period_ka;
	long long      reserved3;
	long long      reserved4;
};

struct bufp_feature_ty {
	union {
		unsigned short feature;
		struct {
			/// Enable buffering time probe
			unsigned char en: 1;
			/// Constance period
			unsigned char fix: 1;
			unsigned char period_ty: 2;
			unsigned char ta: 1;
			unsigned char prep: 3;
			/// When the buffering time probe was done, go to fullboot.
			unsigned char done_en: 1;
			/// When the buffering time probe failed, go to fullboot.
			unsigned char change_en: 1;
			/// When the buffering time probe was done, set automatically
			/// TIMs period.
			unsigned char auto_set: 1;
			/// During UCHK, BUFP doesn't use PS-POLL for measuring AP UC
			/// buffering time.
			unsigned char no_pspoll: 1;
			unsigned char chk_arp: 1;
			unsigned char reserved: 3;
		};
	};

	unsigned short chk_period;

	unsigned short thr_period;
	unsigned char  bcn_timeout_tu;
	/// The BUFP execution count
	unsigned char  retry_cnt;
	/// The transmitted ARPREQ frame count
	unsigned short arpreq_retry_cnt;
	/// The maximum BUFP retry count
	unsigned short max_retry_cnt;
};
...
struct dpm_env {
	...
	unsigned short ap_cap_mask;

	struct fboot_feature_ty fboot;

	unsigned long int_flash_pad;
	unsigned long ext_flash_pad;
	unsigned short cs_rxon_delay;
	// After PTIM finishes receiving UC, PTIM waits to transmit ACK
	// conrresponding to AP retry during uc_delay.
	unsigned short uc_delay;

    /// The sleep preparation time in TU on POR
    unsigned short por_sleep_ready_tu;
    /// The sleep preparation time in TU on Fullboot
    unsigned short fullboot_sleep_ready_tu;

	/// Buffering time pprobe feature
	struct bufp_feature_ty bufp;

    // BTCOEX Priority
    unsigned long btcoex_priority;
};
...
struct dpm_schedule {
    ...
    // The next wakeup time is calculated by last_tim_wakeup_clk when there is
    // a No-TIM function or an external wakeup.
	long long          last_tim_wakeup_clk;
	long long          reserved2;
...
struct dpm_phy_otp_cal {
        ///< OTP Tx Power Offset RR reg#1 ( TEMP , 0x00f8 )
        unsigned long reg1;
        ///< OTP Tx Power Offset RR reg#2 ( HP_POW1,2 0x0261, 0x0263 )
        unsigned long reg2;
};
...
struct dpm_phy_cfg {
	union {
		unsigned long parameters[DPM_PHY_CFG_BUF_SIZE];
		struct {
            ///< OTP Tx Power Offset RR reg#3 ( LP_POW1,2 0x0261, 0x0263 )
            unsigned long otp_cal_reg3;

            unsigned short dbg_module;
            unsigned char dbg_severity;
            unsigned char dbg_print;

			///< VCO LOCK VALUE
			unsigned short vco_lock;
                        unsigned short reserved0;
...
static inline unsigned long dpm_get_env_subnet(struct dpm_param *dpmp)
{
	return (unsigned long) dpmp->env.subnet;
}

static inline void dpm_set_env_subnet(struct dpm_param *dpmp, unsigned long subnet)
{
	dpmp->env.subnet = (long) subnet;
}

static inline unsigned short dpm_get_env_ap_cap_mask(struct dpm_param *dpmp)
{
	return dpmp->env.ap_cap_mask;
}

static inline void dpm_set_env_ap_cap_mask(struct dpm_param *dpmp,
		unsigned short ap_cap_mask)
{
	dpmp->env.ap_cap_mask = ap_cap_mask;
}
...
{
	dpmp->env.freq = (unsigned short) freq;
}
...
{
	dpmp->env.aid = (unsigned short) aid;
}
...
{
	dpmp->env.bcn_int = (unsigned short) bcn_int;
}
...
{
	dpmp->env.half_int = (unsigned short) half_int;
}
		...
		unsigned long force_period)
{
	...
	if (dtim_period == 0)
		dtim_period = (int) force_period;

	if ((int) force_period <= dtim_period)
		force_period = (unsigned long) dtim_period;
	else
		force_period -= (unsigned long) (force_period % (unsigned long) dtim_period);
...
{
	dpmp->env.ptim_timeout_tu = (unsigned short) ptim_timeout_tu;
}
...
static inline unsigned char dpm_get_env_sys_clk3(struct dpm_param *dpmp)
{
	return dpmp->env.sys_clk3;
}

static inline void dpm_set_env_sys_clk3(struct dpm_param *dpmp,
		unsigned char sys_clk3)
{
	dpmp->env.sys_clk3 = sys_clk3;
}
...
static inline unsigned long dpm_get_env_int_flash_pad(struct dpm_param *dpmp)
{
	return dpmp->env.int_flash_pad;
}

static inline void dpm_set_env_int_flash_pad(struct dpm_param *dpmp,
		unsigned long int_flash_pad)
{
	dpmp->env.int_flash_pad = int_flash_pad;
}

static inline unsigned long dpm_get_env_ext_flash_pad(struct dpm_param *dpmp)
{
	return dpmp->env.ext_flash_pad;
}

static inline void dpm_set_env_ext_flash_pad(struct dpm_param *dpmp,
		unsigned long ext_flash_pad)
{
	dpmp->env.ext_flash_pad = ext_flash_pad;
}

static inline unsigned short dpm_get_env_cs_rxon_delay(struct dpm_param *dpmp)
{
	return dpmp->env.cs_rxon_delay;
}

static inline void dpm_set_env_cs_rxon_delay(struct dpm_param *dpmp,
		unsigned short cs_rxon_delay)
{
	dpmp->env.cs_rxon_delay = cs_rxon_delay;
}

static inline unsigned short dpm_get_env_uc_delay(struct dpm_param *dpmp)
{
	return dpmp->env.uc_delay;
}

static inline void dpm_set_env_uc_delay(struct dpm_param *dpmp,
		unsigned short uc_delay)
{
	dpmp->env.uc_delay = uc_delay;
}

static inline unsigned short dpm_get_env_por_sleep_ready_tu(
        struct dpm_param *dpmp)
{
	return dpmp->env.por_sleep_ready_tu;
}

static inline void dpm_set_env_por_sleep_ready_tu(struct dpm_param *dpmp,
		unsigned short por_sleep_ready_tu)
{
	dpmp->env.por_sleep_ready_tu = por_sleep_ready_tu;
}

static inline unsigned short dpm_get_env_fullboot_sleep_ready_tu(
        struct dpm_param *dpmp)
{
	return dpmp->env.fullboot_sleep_ready_tu;
}

static inline void dpm_set_env_fullboot_sleep_ready_tu(struct dpm_param *dpmp,
		unsigned short fullboot_sleep_ready_tu)
{
	dpmp->env.fullboot_sleep_ready_tu = fullboot_sleep_ready_tu;
}

static inline unsigned long dpm_get_env_btcoex_priority(
        struct dpm_param *dpmp)
{
	return dpmp->env.btcoex_priority;
}

static inline void dpm_set_env_btcoex_priority(struct dpm_param *dpmp,
		unsigned long btcoex_priority)
{
	dpmp->env.btcoex_priority = btcoex_priority;
}

static inline unsigned long dpm_get_env_txrate0(struct dpm_param *dpmp)
{
	return dpmp->txr.txrate[0];
}

static inline void dpm_set_env_txrate0(struct dpm_param *dpmp,
		unsigned long txrate)
{
	dpmp->txr.txrate[0] = txrate;
}

static inline unsigned long dpm_get_env_txrate1(struct dpm_param *dpmp)
{
	return dpmp->txr.txrate[1];
}

static inline void dpm_set_env_txrate1(struct dpm_param *dpmp,
		unsigned long txrate)
{
	dpmp->txr.txrate[1] = txrate;
}

static inline unsigned long dpm_get_env_txrate2(struct dpm_param *dpmp)
{
	return dpmp->txr.txrate[2];
}

static inline void dpm_set_env_txrate2(struct dpm_param *dpmp,
		unsigned long txrate)
{
	dpmp->txr.txrate[2] = txrate;
}

static inline unsigned long dpm_get_env_txrate3(struct dpm_param *dpmp)
{
	return dpmp->txr.txrate[3];
}

static inline void dpm_set_env_txrate3(struct dpm_param *dpmp,
		unsigned long txrate)
{
	dpmp->txr.txrate[3] = txrate;
}

static inline unsigned long dpm_get_env_usr_txrate0(struct dpm_param *dpmp)
{
	return dpmp->txr.usr_txrate[0];
}

static inline void dpm_set_env_usr_txrate0(struct dpm_param *dpmp,
		unsigned long usr_txrate)
{
	dpmp->txr.usr_txrate[0] = usr_txrate;
}

static inline unsigned long dpm_get_env_usr_txrate1(struct dpm_param *dpmp)
{
	return dpmp->txr.usr_txrate[1];
}

static inline void dpm_set_env_usr_txrate1(struct dpm_param *dpmp,
		unsigned long usr_txrate)
{
	dpmp->txr.usr_txrate[1] = usr_txrate;
}

static inline unsigned long dpm_get_env_usr_txrate2(struct dpm_param *dpmp)
{
	return dpmp->txr.usr_txrate[2];
}

static inline void dpm_set_env_usr_txrate2(struct dpm_param *dpmp,
		unsigned long usr_txrate)
{
	dpmp->txr.usr_txrate[2] = usr_txrate;
}

static inline unsigned long dpm_get_env_usr_txrate3(struct dpm_param *dpmp)
{
	return dpmp->txr.usr_txrate[3];
}

static inline void dpm_set_env_usr_txrate3(struct dpm_param *dpmp,
		unsigned long usr_txrate)
{
	dpmp->txr.usr_txrate[3] = usr_txrate;
}
...
static inline void dpm_set_env_timp_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.timp.feature = (unsigned short) feat;
}
...
{
	dpmp->env.timp.weaksig_0 = (unsigned char) (weaksig_0 & 0x1);
	dpmp->env.timp.weaksig_1 = (unsigned char) (weaksig_1 & 0x1);
	dpmp->env.timp.update_0 = (unsigned char) (update_0 & 0x1);
	dpmp->env.timp.update_1 = (unsigned char) (update_1 & 0x1);
}
...
static inline void dpm_set_env_timp_chk_wakeup_0(struct dpm_param *dpmp)
{
	dpmp->env.timp.chk_wakeup_0 = 1;
}

static inline void dpm_reset_env_timp_chk_wakeup_0(struct dpm_param *dpmp)
{
	dpmp->env.timp.chk_wakeup_0 = 0;
}

static inline unsigned char dpm_get_env_timp_chk_wakeup_0(struct dpm_param *dpmp)
{
	return dpmp->env.timp.chk_wakeup_0;
}

static inline void dpm_set_env_timp_chk_wakeup_1(struct dpm_param *dpmp)
{
	dpmp->env.timp.chk_wakeup_1 = 1;
}

static inline void dpm_reset_env_timp_chk_wakeup_1(struct dpm_param *dpmp)
{
	dpmp->env.timp.chk_wakeup_1 = 0;
}

static inline unsigned char dpm_get_env_timp_chk_wakeup_1(struct dpm_param *dpmp)
{
	return dpmp->env.timp.chk_wakeup_1;
}
...
static inline void dpm_set_env_tim_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.tim.feature = (unsigned short) feat;
}
...
static inline void dpm_set_env_tim(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned char bcn_sync_clk)
{
	dpmp->env.tim.en = (unsigned char) (en & 0x1);
	dpmp->env.tim.fix = (unsigned char) (fix & 0x1);
	dpmp->env.tim.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.tim.prep = (unsigned char) (prep & 0x7);
	dpmp->env.tim.wait_bcn = (unsigned char) (wait_bcn & 0x1);
	dpmp->env.tim.detect_ssid = (unsigned char) (detect_ssid & 0x1);
	dpmp->env.tim.detect_channel = (unsigned char) (detect_channel & 0x1);
	dpmp->env.tim.dont_care_loss = (unsigned char) (dont_care_loss & 0x1);
	dpmp->env.tim.pbr = (unsigned char) (pbr & 0x1);
	dpmp->env.tim.bcn_sync = (unsigned char) (bcn_sync & 0x1);
...
static inline unsigned char dpm_get_env_tim_ofdm_en(
		struct dpm_param *dpmp)
{
	return dpmp->env.tim.ofdm_en;
}

static inline void dpm_reset_env_tim_ofdm_en(struct dpm_param *dpmp)
{
	dpmp->env.tim.ofdm_en = 0;
}

static inline void dpm_set_env_tim_ofdm_en(struct dpm_param *dpmp)
{
	dpmp->env.tim.ofdm_en = 1;
}
...
{
	dpmp->env.tim.bcn_sync_clk = (unsigned char) bcn_sync_clk;
}
...
static inline void dpm_set_env_bcmc(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned char max)
{
	dpmp->env.bcmc.en = (unsigned char) (en & 0x1);
	dpmp->env.bcmc.morechk = (unsigned char) (morechk & 0x1);
	dpmp->env.bcmc.rx_timeout_control = (unsigned char) (rx_timeout_control & 0x1);
...
static inline void dpm_set_env_ps_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.ps.feature = (unsigned short) feat;
}
...
static inline void dpm_set_env_ps(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned char ps_data_max)
{
	dpmp->env.ps.en = (unsigned char) (en & 0x1);
	dpmp->env.ps.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.ps.ta = (unsigned char) (ta & 0x1);
	dpmp->env.ps.prep = (unsigned char) (prep & 0x3);
...
static inline void dpm_set_env_ps_prep(struct dpm_param *dpmp,
		unsigned char prep)
{
	dpmp->env.ps.prep = (unsigned char) (prep & 0x7);
}
...
static inline void dpm_set_env_uc(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned char max)
{
	dpmp->env.uc.en = (unsigned char) (en & 0x1);
	dpmp->env.uc.morechk = (unsigned char) (morechk & 0x1);
	dpmp->env.uc.rx_timeout_control = (unsigned char) (rx_timeout_control & 0x1);
...
static inline unsigned char dpm_get_env_uc_period_ty(struct dpm_param *dpmp)
{
	return dpmp->env.uc.period_ty;
}

static inline void dpm_reset_env_uc_period_ty(struct dpm_param *dpmp)
{
	dpmp->env.uc.period_ty = 0;
}

static inline void dpm_set_env_uc_period_ty(struct dpm_param *dpmp,
											unsigned char period_ty)
{
	dpmp->env.uc.period_ty = (unsigned char) (period_ty & 0x3);
}
...
static inline unsigned char dpm_get_env_uc_chk_wakeup_en(
		struct dpm_param *dpmp)
{
	return dpmp->env.uc.chk_wakeup_en;
}

static inline void dpm_reset_env_uc_chk_wakeup_en(struct dpm_param *dpmp)
{
	dpmp->env.uc.chk_wakeup_en = 0;
}

static inline void dpm_set_env_uc_chk_wakeup_en(struct dpm_param *dpmp)
{
	dpmp->env.uc.chk_wakeup_en = 1;
}

static inline unsigned char dpm_get_env_uc_retry_en(
		struct dpm_param *dpmp)
{
	return dpmp->env.uc.retry_en;
}

static inline void dpm_reset_env_uc_retry_en(struct dpm_param *dpmp)
{
	dpmp->env.uc.retry_en = 0;
}

static inline void dpm_set_env_uc_retry_en(struct dpm_param *dpmp)
{
	dpmp->env.uc.retry_en = 1;
}
...
static inline unsigned char dpm_get_env_uc_retry_max(
		struct dpm_param *dpmp)
{
	return dpmp->env.uc.retry_max;
}

static inline void dpm_set_env_uc_retry_max(struct dpm_param *dpmp,
		unsigned char retry_max)
{
	dpmp->env.uc.retry_max = retry_max;
}

/*static inline unsigned char dpm_get_env_uc_postprep_tu(
		struct dpm_param *dpmp)
{
	return dpmp->env.uc.postprep_tu;
}

static inline void dpm_set_env_uc_postprep_tu(struct dpm_param *dpmp,
		unsigned char postprep_tu)
{
	dpmp->env.uc.postprep_tu = postprep_tu;
}*/

static inline unsigned long dpm_get_env_uc_chk_period(struct dpm_param *dpmp)
{
	return dpmp->env.uc.chk_period;
}

static inline void dpm_set_env_uc_chk_period(struct dpm_param *dpmp,
												unsigned long chk_period)
{
	dpmp->env.uc.chk_period = chk_period;
}
...
static inline void dpm_set_env_ka_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.ka.feature = (unsigned short) feat;
}
...
{
	dpmp->env.ka.en = (unsigned char) (en & 0x1);
	dpmp->env.ka.fix = (unsigned char) (fix & 0x1);
	dpmp->env.ka.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.ka.prep = (unsigned char) (prep & 0x7);
	dpmp->env.ka.ta = (unsigned char) (ta & 0x1);
	dpmp->env.ka.td = (unsigned char) (td & 0x1);
...
static inline unsigned char dpm_get_env_ka_tr(struct dpm_param *dpmp)
{
	return dpmp->env.ka.tr;
}

static inline void dpm_reset_env_ka_tr(struct dpm_param *dpmp)
{
	dpmp->env.ka.tr = 0;
}

static inline void dpm_set_env_ka_tr(struct dpm_param *dpmp)
{
	dpmp->env.ka.tr = 1;
}
...
static inline unsigned long dpm_get_env_ka_period_ms(struct dpm_param *dpmp)
{
	return dpmp->env.ka.period_ms;
}

static inline void dpm_set_env_ka_period_ms(struct dpm_param *dpmp,
		unsigned long period_ms)
{
	dpmp->env.ka.period_ms = period_ms;
}
...
static inline void dpm_set_env_arp_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.arp.feature = (unsigned short) feat;
}
...
{
	dpmp->env.arp.en = (unsigned char) (en & 0x1);
	dpmp->env.arp.fix = (unsigned char) (fix & 0x1);
	dpmp->env.arp.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.arp.prep = (unsigned char) (prep & 0x7);
	dpmp->env.arp.ta = (unsigned char) (ta & 0x1);
	dpmp->env.arp.timeout_tu = timeout_tu;
	dpmp->env.arp.retry = retry;
	dpmp->env.arp.period = period;
	dpmp->env.arp.type = (unsigned char) (type & 0x3);
}
...
static inline void dpm_set_env_arp_type(struct dpm_param *dpmp,
		unsigned char ty)
{
	dpmp->env.arp.type = (unsigned char) (ty & 0x3);
}
...
static inline unsigned short dpm_get_env_arp_period_ka(struct dpm_param *dpmp)
{
	return dpmp->env.arp.period_ka;
}

static inline void dpm_set_env_arp_period_ka(struct dpm_param *dpmp,
		unsigned short period_ka)
{
	dpmp->env.arp.period_ka = period_ka;
}
...
static inline void dpm_set_env_arpresp_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.arpresp.feature = (unsigned short) feat;
}
...
static inline void dpm_set_env_arpresp(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned long period)
{
	dpmp->env.arpresp.en = (unsigned char) (en & 0x1);
	dpmp->env.arpresp.fix = (unsigned char) (fix & 0x1);
	dpmp->env.arpresp.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.arpresp.prep = (unsigned char) (prep & 0x7);
	dpmp->env.arpresp.ta = (unsigned char) (ta & 0x1);
...
static inline void dpm_set_env_udph_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.udph.feature = (unsigned short) feat;
}
...
{
	dpmp->env.udph.en = (unsigned char) (en & 0x1);
	dpmp->env.udph.fix = (unsigned char) (fix & 0x1);
	dpmp->env.udph.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.udph.prep = (unsigned char) (prep & 0x7);
	dpmp->env.udph.ta = (unsigned char) (ta & 0x1);
	dpmp->env.udph.timeout_tu = timeout_tu;
	dpmp->env.udph.retry = retry;
	dpmp->env.udph.period = period;
	dpmp->env.udph.type = type;
	dpmp->env.udph.payload_len = (unsigned int) (payload_len & 0xffffff);
}
...
static inline unsigned short dpm_get_env_udph_period_ka(struct dpm_param *dpmp)
{
	return dpmp->env.udph.period_ka;
}

static inline void dpm_set_env_udph_period_ka(struct dpm_param *dpmp,
		unsigned short period_ka)
{
	dpmp->env.udph.period_ka = period_ka;
}
...
static inline void dpm_set_env_setps_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.setps.feature = (unsigned short) feat;
}
...
static inline void dpm_set_env_setps(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned long period)
{
	dpmp->env.setps.en = (unsigned char) (en & 0x1);
	dpmp->env.setps.fix = (unsigned char) (fix & 0x1);
	dpmp->env.setps.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.setps.prep = (unsigned char) (prep & 0x7);
	dpmp->env.setps.ta = (unsigned char) (ta & 0x1);
...
static inline unsigned char dpm_get_env_setps_tr(struct dpm_param *dpmp)
{
	return dpmp->env.setps.tr;
}

static inline void dpm_reset_env_setps_tr(struct dpm_param *dpmp)
{
	dpmp->env.setps.tr = 0;
}

static inline void dpm_set_env_setps_tr(struct dpm_param *dpmp)
{
	dpmp->env.setps.tr = 1;
}
 ...
 * BOR feature
 */
static inline unsigned char dpm_get_env_bor_en(struct dpm_param *dpmp)
{
	return dpmp->env.bor.en;
}

static inline void dpm_set_env_bor_en(struct dpm_param *dpmp, unsigned char en)
{
	dpmp->env.bor.en = (unsigned char) (en & 0x1f);
}

static inline unsigned char dpm_get_env_bor_reset_en(struct dpm_param *dpmp)
{
	return dpmp->env.bor.reset_en;
}

static inline void dpm_set_env_bor_reset_en(struct dpm_param *dpmp, unsigned char reset_en)
{
	dpmp->env.bor.reset_en = (unsigned char) (reset_en & 0x1f);
}
...
static inline void dpm_set_env_arpreq_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.arpreq.feature = (unsigned short) feat;
}
...
{
	dpmp->env.arpreq.en = (unsigned char) (en & 0x1);
	dpmp->env.arpreq.fix = (unsigned char) (fix & 0x1);
	dpmp->env.arpreq.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.arpreq.prep = (unsigned char) (prep & 0x7);
	dpmp->env.arpreq.ta = (unsigned char) (ta & 0x1);
	dpmp->env.arpreq.retry = retry;
	dpmp->env.arpreq.type = (unsigned char) (type & 0x3);
...
static inline void dpm_set_env_arpreq_type(struct dpm_param *dpmp,
		unsigned char ty)
{
	dpmp->env.arpreq.type = (unsigned char) (ty & 0x3);
}
...
static inline unsigned short dpm_get_env_arpreq_period_ka(struct dpm_param *dpmp)
{
	return dpmp->env.arpreq.period_ka;
}

static inline void dpm_set_env_arpreq_period_ka(struct dpm_param *dpmp,
		unsigned short period_ka)
{
	dpmp->env.arpreq.period_ka = period_ka;
}
...
static inline void dpm_set_env_deauth_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.deauth.feature = (unsigned short) feat;
}
...
static inline void dpm_set_env_deauth(struct dpm_param *dpmp, unsigned char en,
		...
		unsigned long period)
{
	dpmp->env.deauth.en = (unsigned char) (en & 0x1);
	dpmp->env.deauth.fix = (unsigned char) (fix & 0x1);
	dpmp->env.deauth.period_ty = (unsigned char) (period_ty & 0x3);
	dpmp->env.deauth.prep = (unsigned char) (prep & 0x7);
	dpmp->env.deauth.ta = (unsigned char) (ta & 0x1);
	dpmp->env.deauth.retry = retry;
	dpmp->env.deauth.type = (unsigned char) (type & 0x1);
...
static inline void dpm_set_env_deauth_type(struct dpm_param *dpmp,
		unsigned char ty)
{
	dpmp->env.deauth.type = (unsigned char) (ty & 0x1);
}
...
static inline unsigned short dpm_get_env_deauth_period_ka(struct dpm_param *dpmp)
{
	return dpmp->env.deauth.period_ka;
}

static inline void dpm_set_env_deauth_period_ka(struct dpm_param *dpmp,
		unsigned short period_ka)
{
	dpmp->env.deauth.period_ka = period_ka;
}
 ...
 * TCPKA feature
 */
static inline unsigned long dpm_get_env_tcpka_feature(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.feature;
}

static inline void dpm_set_env_tcpka_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.tcpka.feature = (unsigned short) feat;
}

static inline void dpm_set_env_tcpka(struct dpm_param *dpmp, unsigned char en,
		unsigned char fix, unsigned char period_ty, unsigned char prep,
		unsigned char ta, unsigned char timeout_tu, unsigned char retry,
		unsigned long period)
{
	dpmp->env.tcpka.en = (unsigned char) (en & 0x1);
	dpmp->env.tcpka.fix = (unsigned char) (fix & 0x1);
	dpmp->env.tcpka.period_ty  = (unsigned char) (period_ty & 0x3);
	dpmp->env.tcpka.prep = (unsigned char) (prep & 0x7);
	dpmp->env.tcpka.ta  = (unsigned char) (ta & 0x1);
	dpmp->env.tcpka.timeout_tu = timeout_tu;
	dpmp->env.tcpka.retry = retry;
	dpmp->env.tcpka.period = period;
}

static inline unsigned char dpm_get_env_tcpka_en(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.en;
}

static inline void dpm_reset_env_tcpka_en(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.en = 0;
}

static inline void dpm_set_env_tcpka_en(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.en = 1;
}

static inline unsigned char dpm_get_env_tcpka_fix(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.fix;
}

static inline void dpm_reset_env_tcpka_fix(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.fix = 0;
}

static inline void dpm_set_env_tcpka_fix(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.fix = 1;
}

static inline unsigned char dpm_get_env_tcpka_period_ty(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.period_ty;
}

static inline void dpm_reset_env_tcpka_period_ty(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.period_ty = 0;
}

static inline void dpm_set_env_tcpka_period_ty(struct dpm_param *dpmp,
		unsigned char period_ty)
{
	dpmp->env.tcpka.period_ty = (unsigned char) (period_ty & 0x3);
}

static inline unsigned char dpm_get_env_tcpka_prep(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.prep;
}

static inline void dpm_set_env_tcpka_prep(struct dpm_param *dpmp,
		unsigned char prep)
{
	dpmp->env.tcpka.prep = (unsigned char) (prep & 0x7);
}

static inline unsigned char dpm_get_env_tcpka_ta(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.ta;
}

static inline void dpm_reset_env_tcpka_ta(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.ta = 0;
}

static inline void dpm_set_env_tcpka_ta(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.ta = 1;
}

static inline unsigned char dpm_get_env_tcpka_retry(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.retry;
}

static inline void dpm_set_env_tcpka_retry(struct dpm_param *dpmp,
		unsigned char retry)
{
	dpmp->env.tcpka.retry = retry;
}

static inline unsigned long dpm_get_env_tcpka_period(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.period;
}

static inline void dpm_set_env_tcpka_period(struct dpm_param *dpmp,
		unsigned long period)
{
	dpmp->env.tcpka.period = period;
}

static inline unsigned char dpm_get_env_tcpka_timeout_tu(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.timeout_tu;
}

static inline void dpm_set_env_tcpka_timeout_tu(struct dpm_param *dpmp,
		unsigned char timeout_tu)
{
	dpmp->env.tcpka.timeout_tu = timeout_tu;
}
...
/* Not used */
static inline unsigned char dpm_get_env_tcpka_type(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.type;
}

/* Not used */
static inline void dpm_set_env_tcpka_type(struct dpm_param *dpmp,
		unsigned char type)
{
	dpmp->env.tcpka.type = type;
}

/* Not used */
static inline unsigned int dpm_get_env_tcpka_payload_len(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.payload_len;
}

/* Not used */
static inline void dpm_set_env_tcpka_payload_len(struct dpm_param *dpmp,
		unsigned int payload_len)
{
	dpmp->env.tcpka.payload_len = (unsigned int) (payload_len & 0xffffff);
}

static inline unsigned long dpm_get_env_tcpka_target_ip(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.target_ip;
}

static inline void dpm_set_env_tcpka_target_ip(struct dpm_param *dpmp,
		unsigned long target_ip)
{
	dpmp->env.tcpka.target_ip = target_ip;
}

static inline unsigned short dpm_get_env_tcpka_sport(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.sport;
}

static inline void dpm_set_env_tcpka_sport(struct dpm_param *dpmp,
		unsigned short sport)
{
	dpmp->env.tcpka.sport = sport;
}

static inline unsigned short dpm_get_env_tcpka_dport(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.dport;
}

static inline void dpm_set_env_tcpka_dport(struct dpm_param *dpmp,
		unsigned short dport)
{
	dpmp->env.tcpka.dport = dport;
}

static inline unsigned short *dpm_get_env_tcpka_target_mac(struct dpm_param *dpmp)
{
	return (unsigned short *) &dpmp->env.tcpka.target_mac[0];
}

static inline void dpm_set_env_tcpka_target_mac(struct dpm_param *dpmp,
		unsigned char *target_mac)
{
	unsigned char *tcpka_target_mac =
		(unsigned char *) &dpmp->env.tcpka.target_mac[0];
	int i;

	for (i = 0; i < DPM_MAC_LEN; i++)
		tcpka_target_mac[i] = target_mac[i];
}

static inline unsigned long dpm_get_env_tcpka_user_data(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.user_data;
}

static inline void dpm_set_env_tcpka_user_data(struct dpm_param *dpmp,
		unsigned long user_data)
{
	dpmp->env.tcpka.user_data = user_data;
}

static inline unsigned short dpm_get_env_tcpka_period_ka(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.period_ka;
}

static inline void dpm_set_env_tcpka_period_ka(struct dpm_param *dpmp,
		unsigned short period_ka)
{
	dpmp->env.tcpka.period_ka = period_ka;
}

static inline unsigned char dpm_get_env_tcpka_chkport_en(struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.chkport_en;
}

static inline void dpm_reset_env_tcpka_chkport_en(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.chkport_en = 0;
}

static inline void dpm_set_env_tcpka_chkport_en(struct dpm_param *dpmp)
{
	dpmp->env.tcpka.chkport_en = 1;
}

static inline unsigned char dpm_get_env_tcpka_chkport_period_ty(
        struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.chkport_period_ty;
}

static inline void dpm_set_env_tcpka_chkport_period_ty(struct dpm_param *dpmp,
		unsigned char chkport_period_ty)
{
	dpmp->env.tcpka.chkport_period_ty = (unsigned char) (chkport_period_ty & 0x3);
}

static inline unsigned short dpm_get_env_tcpka_chkport_dport(
        struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.chkport_dport;
}

static inline void dpm_set_env_tcpka_chkport_dport(struct dpm_param *dpmp,
		unsigned short chkport_dport)
{
	dpmp->env.tcpka.chkport_dport = chkport_dport;
}

static inline unsigned char dpm_get_env_tcpka_chkport_period_ty_ka(
        struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.chkport_period_ty_ka;
}

static inline void dpm_reset_env_tcpka_chkport_period_ty_ka(
        struct dpm_param *dpmp)
{
	dpmp->env.tcpka.chkport_period_ty_ka = 0;
}

static inline void dpm_set_env_tcpka_chkport_period_ty_ka(
        struct dpm_param *dpmp)
{
	dpmp->env.tcpka.chkport_period_ty_ka = 1;
}

static inline unsigned short dpm_get_env_tcpka_chkport_period(
        struct dpm_param *dpmp)
{
	return dpmp->env.tcpka.chkport_period;
}

static inline void dpm_set_env_tcpka_chkport_period(struct dpm_param *dpmp,
		unsigned short chkport_period)
{
	dpmp->env.tcpka.chkport_period = (unsigned short) (chkport_period & 0xfff);
}
 ...
 * Fboot
 */
static inline unsigned int dpm_get_env_fboot_period0_100ms(struct dpm_param *dpmp)
{
	return dpmp->env.fboot.period0_100ms;
}

static inline void dpm_set_env_fboot_period0_100ms(struct dpm_param *dpmp,
		unsigned int period0_100ms)
{
	dpmp->env.fboot.period0_100ms = (unsigned int) (period0_100ms & 0x3fffff);
}

static inline unsigned int dpm_get_env_fboot_period0_drift(struct dpm_param *dpmp)
{
	return dpmp->env.fboot.period0_drift;
}

static inline void dpm_set_env_fboot_period0_drift(struct dpm_param *dpmp,
		unsigned int period0_drift)
{
	dpmp->env.fboot.period0_drift = (unsigned int) (period0_drift & 0x3ff);
}

static inline unsigned int dpm_get_env_fboot_period1_100ms(struct dpm_param *dpmp)
{
	return dpmp->env.fboot.period1_100ms;
}

static inline void dpm_set_env_fboot_period1_100ms(struct dpm_param *dpmp,
		unsigned int period1_100ms)
{
	dpmp->env.fboot.period1_100ms = (unsigned int) (period1_100ms & 0x3fffff);
}

static inline unsigned int dpm_get_env_fboot_period1_drift(struct dpm_param *dpmp)
{
	return dpmp->env.fboot.period1_drift;
}

static inline void dpm_set_env_fboot_period1_drift(struct dpm_param *dpmp,
		unsigned int period1_drift)
{
	dpmp->env.fboot.period1_drift = (unsigned int) (period1_drift & 0x3ff);
}



/*
 * Buffering time probe feature
 */
static inline unsigned long dpm_get_env_bufp_feature(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.feature;
}

static inline void dpm_set_env_bufp_feature(struct dpm_param *dpmp,
		unsigned long feat)
{
	dpmp->env.bufp.feature = (unsigned short) feat;
}

static inline void dpm_set_env_bufp(struct dpm_param *dpmp, unsigned char en,
		unsigned char fix, unsigned char period_ty, unsigned char prep,
		unsigned char ta, unsigned char done_en, unsigned char change_en,
		unsigned auto_set, unsigned char chk_arp, unsigned char retry_cnt,
		unsigned short chk_period)
{
	dpmp->env.bufp.en = (unsigned char) (en & 0x1);
	dpmp->env.bufp.fix = (unsigned char) (fix & 0x1);
	dpmp->env.bufp.period_ty  = (unsigned char) (period_ty & 0x3);
	dpmp->env.bufp.prep = (unsigned char) (prep & 0x7);
	dpmp->env.bufp.ta  = (unsigned char) (ta & 0x1);
	dpmp->env.bufp.done_en  = (unsigned char) (done_en & 0x1);
	dpmp->env.bufp.change_en = (unsigned char) (change_en & 0x1);
	dpmp->env.bufp.auto_set = (unsigned char) (auto_set & 0x1);
	dpmp->env.bufp.chk_arp = (unsigned char) (chk_arp & 0x1);
	dpmp->env.bufp.retry_cnt = retry_cnt;
	dpmp->env.bufp.chk_period = chk_period;
}

static inline unsigned char dpm_get_env_bufp_en(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.en;
}

static inline void dpm_reset_env_bufp_en(struct dpm_param *dpmp)
{
	dpmp->env.bufp.en = 0;
}

static inline void dpm_set_env_bufp_en(struct dpm_param *dpmp)
{
	dpmp->env.bufp.en = 1;
}

static inline unsigned char dpm_get_env_bufp_fix(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.fix;
}

static inline void dpm_reset_env_bufp_fix(struct dpm_param *dpmp)
{
	dpmp->env.bufp.fix = 0;
}

static inline void dpm_set_env_bufp_fix(struct dpm_param *dpmp)
{
	dpmp->env.bufp.fix = 1;
}

static inline unsigned char dpm_get_env_bufp_period_ty(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.period_ty;
}

static inline void dpm_reset_env_bufp_period_ty(struct dpm_param *dpmp)
{
	dpmp->env.bufp.period_ty = 0;
}

static inline void dpm_set_env_bufp_period_ty(struct dpm_param *dpmp,
		unsigned char period_ty)
{
	dpmp->env.bufp.period_ty = (unsigned char) (period_ty & 0x3);
}

static inline unsigned char dpm_get_env_bufp_prep(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.prep;
}

static inline void dpm_set_env_bufp_prep(struct dpm_param *dpmp,
		unsigned char prep)
{
	dpmp->env.bufp.prep = (unsigned char) (prep & 0x7);
}

static inline unsigned char dpm_get_env_bufp_ta(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.ta;
}

static inline void dpm_reset_env_bufp_ta(struct dpm_param *dpmp)
{
	dpmp->env.bufp.ta = 0;
}

static inline void dpm_set_env_bufp_ta(struct dpm_param *dpmp)
{
	dpmp->env.bufp.ta = 1;
}

static inline unsigned char dpm_get_env_bufp_done_en(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.done_en;
}

static inline void dpm_reset_env_bufp_done_en(struct dpm_param *dpmp)
{
	dpmp->env.bufp.done_en = 0;
}

static inline void dpm_set_env_bufp_done_en(struct dpm_param *dpmp)
{
	dpmp->env.bufp.done_en = 1;
}

static inline unsigned char dpm_get_env_bufp_change_en(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.change_en;
}

static inline void dpm_reset_env_bufp_change_en(struct dpm_param *dpmp)
{
	dpmp->env.bufp.change_en = 0;
}

static inline void dpm_set_env_bufp_change_en(struct dpm_param *dpmp)
{
	dpmp->env.bufp.change_en = 1;
}

static inline unsigned char dpm_get_env_bufp_auto_set(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.auto_set;
}

static inline void dpm_reset_env_bufp_auto_set(struct dpm_param *dpmp)
{
	dpmp->env.bufp.auto_set = 0;
}

static inline void dpm_set_env_bufp_auto_set(struct dpm_param *dpmp)
{
	dpmp->env.bufp.auto_set = 1;
}

static inline unsigned char dpm_get_env_bufp_no_pspoll(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.no_pspoll;
}

static inline void dpm_reset_env_bufp_no_pspoll(struct dpm_param *dpmp)
{
	dpmp->env.bufp.no_pspoll = 0;
}

static inline void dpm_set_env_bufp_no_pspoll(struct dpm_param *dpmp)
{
	dpmp->env.bufp.no_pspoll = 1;
}

static inline unsigned char dpm_get_env_bufp_chk_arp(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.chk_arp;
}

static inline void dpm_reset_env_bufp_chk_arp(struct dpm_param *dpmp)
{
	dpmp->env.bufp.chk_arp = 0;
}

static inline void dpm_set_env_bufp_chk_arp(struct dpm_param *dpmp)
{
	dpmp->env.bufp.chk_arp = 1;
}

static inline unsigned char dpm_get_env_bufp_retry_cnt(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.retry_cnt;
}

static inline void dpm_set_env_bufp_retry_cnt(struct dpm_param *dpmp,
												unsigned char retry_cnt)
{
	dpmp->env.bufp.retry_cnt = retry_cnt;
}

static inline unsigned short dpm_get_env_bufp_chk_period(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.chk_period;
}

static inline void dpm_set_env_bufp_chk_period(struct dpm_param *dpmp,
												unsigned short chk_period)
{
	dpmp->env.bufp.chk_period = chk_period;
}

static inline unsigned short dpm_get_env_bufp_thr_period(struct dpm_param *dpmp)
{
	return dpmp->env.bufp.thr_period;
}

static inline void dpm_set_env_bufp_thr_period(struct dpm_param *dpmp,
												unsigned short thr_period)
{
	dpmp->env.bufp.thr_period = thr_period;
}

static inline unsigned char dpm_get_env_bufp_bcn_timeout_tu(
		struct dpm_param *dpmp)
{
	return dpmp->env.bufp.bcn_timeout_tu;
}

static inline void dpm_set_env_bufp_bcn_timeout_tu(struct dpm_param *dpmp,
												unsigned char bcn_timeout_tu)
{
	dpmp->env.bufp.bcn_timeout_tu = bcn_timeout_tu;
}

static inline unsigned short dpm_get_env_bufp_arpreq_retry_cnt(
												struct dpm_param *dpmp)
{
	return dpmp->env.bufp.arpreq_retry_cnt;
}

static inline void dpm_set_env_bufp_arpreq_retry_cnt(struct dpm_param *dpmp,
												unsigned short arpreq_retry_cnt)
{
	dpmp->env.bufp.arpreq_retry_cnt = arpreq_retry_cnt;
}

static inline unsigned short dpm_get_env_bufp_max_retry_cnt(
		struct dpm_param *dpmp)
{
	return dpmp->env.bufp.max_retry_cnt;
}

static inline void dpm_set_env_bufp_max_retry_cnt(struct dpm_param *dpmp,
												unsigned short max_retry_cnt)
{
	dpmp->env.bufp.max_retry_cnt = max_retry_cnt;
}
...
{
	return (unsigned char) dpmp->st.pm_status;
}
...
static inline unsigned char dpm_get_st_txretry_ka_en(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.txretry_ka_en;
}

static inline void dpm_reset_st_txretry_ka_en(struct dpm_param *dpmp)
{
	dpmp->st.txretry_ka_en = 0;
}

static inline void dpm_set_st_txretry_ka_en(struct dpm_param *dpmp)
{
	dpmp->st.txretry_ka_en = 1;
}

static inline unsigned char dpm_get_st_txretry_setps_en(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.txretry_setps_en;
}

static inline void dpm_reset_st_txretry_setps_en(struct dpm_param *dpmp)
{
	dpmp->st.txretry_setps_en = 0;
}

static inline void dpm_set_st_txretry_setps_en(struct dpm_param *dpmp)
{
	dpmp->st.txretry_setps_en = 1;
}
...
{
	dpmp->st.rssi_status = (unsigned char) (rssi_status & 0x1);
}

static inline unsigned char dpm_get_st_bufp_uc_detect(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.bufp_uc_detect;
}

static inline void dpm_reset_st_bufp_uc_detect(struct dpm_param *dpmp)
{
	dpmp->st.bufp_uc_detect = 0;
}

static inline void dpm_set_st_bufp_uc_detect(struct dpm_param *dpmp)
{
	dpmp->st.bufp_uc_detect = 1;
}

static inline unsigned char dpm_get_st_bufp_probe_pending(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.bufp_probe_pending;
}

static inline void dpm_reset_st_bufp_probe_pending(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_pending = 0;
}

static inline void dpm_set_st_bufp_probe_pending(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_pending = 1;
}

static inline unsigned char dpm_get_st_bufp_arp_status(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.bufp_arp_status;
}

static inline void dpm_reset_st_bufp_arp_status(struct dpm_param *dpmp)
{
	dpmp->st.bufp_arp_status = 0;
}

static inline void dpm_set_st_bufp_arp_status(struct dpm_param *dpmp)
{
	dpmp->st.bufp_arp_status = 1;
}

static inline unsigned char dpm_get_st_bufp_probe_status(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.bufp_probe_status;
}

static inline void dpm_reset_st_bufp_probe_status(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_status = 0;
}

static inline void dpm_set_st_bufp_probe_status(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_status = 1;
}

static inline unsigned char dpm_get_st_bufp_status(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.bufp_status;
}

static inline void dpm_reset_st_bufp_status(struct dpm_param *dpmp)
{
	dpmp->st.bufp_status = DPM_BUFP_READY;
}

/*static inline void dpm_next_st_bufp_status(struct dpm_param *dpmp)
{
	if (dpmp->st.bufp_status == DPM_BUFP_SUCCESS ||
									dpmp->st.bufp_status == DPM_BUFP_FAILURE)
		return;

	dpmp->st.bufp_status++;
}*/

static inline void dpm_set_st_bufp_status(struct dpm_param *dpmp,
		unsigned char bufp_status)
{
	dpmp->st.bufp_status = (unsigned char) (bufp_status & 0x7);
}

static inline unsigned char dpm_get_st_uc_retry(struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.uc_retry;
}

static inline void dpm_reset_st_uc_retry(struct dpm_param *dpmp)
{
	dpmp->st.uc_retry = 0;
}

static inline void dpm_set_st_uc_retry(struct dpm_param *dpmp)
{
	dpmp->st.uc_retry = 1;
}
...
{
	return (unsigned char) dpmp->st.ps_type;
}

static inline void dpm_set_st_ps_type(struct dpm_param *dpmp,
		unsigned char type)
{
	dpmp->st.ps_type = (unsigned char) (type & 0x3);
}

static inline unsigned char dpm_get_st_determined_ps_type(
		struct dpm_param *dpmp)
{
	return (unsigned char) dpmp->st.determined_ps_type;
}
...
static inline unsigned char dpm_get_st_bcn_prem_us(struct dpm_param *dpmp)
{
	return dpmp->st.bcn_prem_us;
}

static inline void dpm_set_st_bcn_prem_us(struct dpm_param *dpmp,
		unsigned char bcn_prem_us)
{
	dpmp->st.bcn_prem_us = bcn_prem_us;
}
...
static inline unsigned char dpm_get_st_primary_ch(struct dpm_param *dpmp)
{
	return dpmp->st.primary_ch;
}

static inline void dpm_set_st_primary_ch(struct dpm_param *dpmp,
		unsigned char primary_ch)
{
	dpmp->st.primary_ch = primary_ch;
}

static inline unsigned char dpm_get_st_dcdc_st_ctrl(struct dpm_param *dpmp)
{
	return dpmp->st.dcdc_st_ctrl;
}

static inline void dpm_set_st_dcdc_st_ctrl(struct dpm_param *dpmp,
		unsigned char dcdc_st_ctrl)
{
	dpmp->st.dcdc_st_ctrl = dcdc_st_ctrl;
}

static inline unsigned short dpm_get_st_tcp_tx_time(struct dpm_param *dpmp)
{
	return dpmp->st.tcp_tx_time;
}

static inline void dpm_set_st_tcp_tx_time(struct dpm_param *dpmp,
		unsigned short tcp_tx_time)
{
	if (dpmp->st.tcp_tx_time > tcp_tx_time || dpmp->st.tcp_tx_time == 0)
		dpmp->st.tcp_tx_time = tcp_tx_time;
}

static inline unsigned long dpm_get_st_tcp_rx_crc(struct dpm_param *dpmp)
{
	return dpmp->st.tcp_rx_crc;
}

static inline void dpm_set_st_tcp_rx_crc(struct dpm_param *dpmp,
		unsigned long tcp_rx_crc)
{
	dpmp->st.tcp_rx_crc = tcp_rx_crc;
}

static inline unsigned short dpm_get_st_tcp_rx_len(struct dpm_param *dpmp)
{
	return dpmp->st.tcp_rx_len;
}

static inline void dpm_set_st_tcp_rx_len(struct dpm_param *dpmp,
		unsigned short tcp_rx_len)
{
	dpmp->st.tcp_rx_len = tcp_rx_len;
}

/*static inline unsigned short dpm_get_st_tcp_response_time(struct dpm_param *dpmp)
{
	return dpmp->st.tcp_response_time;
}

static inline void dpm_set_st_tcp_response_time(struct dpm_param *dpmp,
		unsigned short tcp_response_time)
{
	if (dpmp->st.tcp_response_time > tcp_response_time ||
			dpmp->st.tcp_response_time == 0)
		dpmp->st.tcp_response_time = tcp_response_time;
}*/

/*static inline unsigned long dpm_get_st_tcp_start(struct dpm_param *dpmp)
{
	return dpmp->st.tcp_start;
}

static inline void dpm_set_st_tcp_start(struct dpm_param *dpmp,
		unsigned long tcp_start)
{
	dpmp->st.tcp_start = tcp_start;
}

static inline unsigned long dpm_get_st_tcp_end(struct dpm_param *dpmp)
{
	return dpmp->st.tcp_end;
}

static inline void dpm_set_st_tcp_end(struct dpm_param *dpmp,
		unsigned long tcp_end)
{
	dpmp->st.tcp_end = tcp_end;
}*/

static inline unsigned short dpm_get_st_ap_cap(struct dpm_param *dpmp)
{
	return dpmp->st.ap_cap;
}

static inline void dpm_set_st_ap_cap(struct dpm_param *dpmp,
		unsigned short ap_cap)
{
	dpmp->st.ap_cap = ap_cap;
}
...
static inline unsigned short dpm_get_st_rx_on_us_round(struct dpm_param *dpmp)
{
	return dpmp->st.rx_on_us_round;
}

static inline void dpm_reset_st_rx_on_us_round(struct dpm_param *dpmp)
{
	dpmp->st.rx_on_us_round = 0;
}

// rx_on_us may be long type.
static inline void dpm_set_st_rx_on_us_round(struct dpm_param *dpmp,
		unsigned long rx_on_us)
{
	dpmp->st.rx_on_us_round += (unsigned short) rx_on_us;
	dpmp->st.rx_on_us_round &= 0x03ff;
}

static inline unsigned short dpm_update_st_rx_on_us_round(
		struct dpm_param *dpmp, unsigned long rx_on_us)
{
	return (unsigned short) (dpmp->st.rx_on_us_round + (rx_on_us & 0x03ff)) >> 10;
}

static inline unsigned char dpm_get_st_uc_retry_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.uc_retry_cnt;
}

static inline void dpm_reset_st_uc_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.uc_retry_cnt = 0;
}

static inline void dpm_set_st_uc_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.uc_retry_cnt++;
}

static inline unsigned long dpm_get_st_cs_on_us(struct dpm_param *dpmp)
{
	return dpmp->st.cs_on_us;
}

static inline void dpm_set_st_cs_on_us(struct dpm_param *dpmp,
		unsigned long cs_on_us)
{
	dpmp->st.cs_on_us = cs_on_us;
}

static inline unsigned long dpm_get_st_rx_on_us(struct dpm_param *dpmp)
{
	return dpmp->st.rx_on_us;
}

static inline void dpm_set_st_rx_on_us(struct dpm_param *dpmp,
		unsigned long rx_on_us)
{
	dpmp->st.rx_on_us = rx_on_us;
}

static inline unsigned long dpm_get_st_uc_end_us(struct dpm_param *dpmp)
{
	return dpmp->st.uc_end_us;
}

static inline void dpm_set_st_uc_end_us(struct dpm_param *dpmp,
		unsigned long uc_end_us)
{
	dpmp->st.uc_end_us = uc_end_us;
}

static inline unsigned long dpm_get_st_uchk_status(struct dpm_param *dpmp)
{
	return dpmp->st.uchk_status;
}

static inline void dpm_clear_st_uchk_status(struct dpm_param *dpmp,
		unsigned long uchk_status)
{
	dpmp->st.uchk_status &= ~uchk_status;
}

static inline void dpm_reset_st_uchk_status(struct dpm_param *dpmp)
{
	dpmp->st.uchk_status = 0;
}

static inline void dpm_set_st_uchk_status(struct dpm_param *dpmp,
		unsigned long uchk_status)
{
	dpmp->st.uchk_status |= uchk_status;
}

static inline unsigned long dpm_get_st_fullboot_sleep_us(struct dpm_param *dpmp)
{
	return dpmp->st.fullboot_sleep_us;
}

static inline void dpm_set_st_fullboot_sleep_us(struct dpm_param *dpmp,
		unsigned long sleep_us)
{
	dpmp->st.fullboot_sleep_us = sleep_us;
}

static inline unsigned long dpm_get_st_fullboot_max_sleep_us(
		struct dpm_param *dpmp)
{
	return dpmp->st.fullboot_max_sleep_us;
}

static inline void dpm_set_st_fullboot_max_sleep_us(struct dpm_param *dpmp,
		unsigned long sleep_us)
{
	if (sleep_us > dpmp->st.fullboot_max_sleep_us)
		dpmp->st.fullboot_max_sleep_us = sleep_us;
}

static inline unsigned long dpm_get_st_fullboot_min_sleep_us(
		struct dpm_param *dpmp)
{
	return dpmp->st.fullboot_min_sleep_us;
}

static inline void dpm_set_st_fullboot_min_sleep_us(struct dpm_param *dpmp,
		unsigned long sleep_us)
{
	if (sleep_us < dpmp->st.fullboot_min_sleep_us)
		dpmp->st.fullboot_min_sleep_us = sleep_us;
}

static inline unsigned short dpm_get_st_bufp_chk_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_chk_cnt;
}

static inline void dpm_inc_st_bufp_chk_cnt(struct dpm_param *dpmp,
		unsigned short cnt)
{
	dpmp->st.bufp_chk_cnt += cnt;
}

static inline void dpm_reset_st_bufp_chk_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_chk_cnt = 0;
}

static inline unsigned short dpm_get_st_bufp_done_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_done_cnt;
}

static inline void dpm_set_st_bufp_done_cnt(struct dpm_param *dpmp,
		unsigned short bufp_done_cnt)
{
	dpmp->st.bufp_done_cnt = bufp_done_cnt;
}

static inline unsigned short dpm_get_st_bufp_max_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_max_cnt;
}

static inline void dpm_reset_st_bufp_max_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_max_cnt = 0;
}

static inline void dpm_set_st_bufp_max_cnt(struct dpm_param *dpmp,
		unsigned short bufp_done_cnt)
{
	if (dpmp->st.bufp_max_cnt < bufp_done_cnt)
		dpmp->st.bufp_max_cnt = bufp_done_cnt;
}

static inline unsigned short dpm_get_st_bufp_min_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_min_cnt;
}

static inline void dpm_reset_st_bufp_min_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_min_cnt = 0xffff;
}

static inline void dpm_set_st_bufp_min_cnt(struct dpm_param *dpmp,
		unsigned short bufp_done_cnt)
{
	if (dpmp->st.bufp_min_cnt > bufp_done_cnt)
		dpmp->st.bufp_min_cnt = bufp_done_cnt;
}

static inline unsigned short dpm_get_st_bufp_retry_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_retry_cnt;
}

static inline void dpm_dec_st_bufp_retry_cnt(struct dpm_param *dpmp)
{
	if (dpmp->st.bufp_retry_cnt)
		dpmp->st.bufp_retry_cnt--;
}

static inline void dpm_inc_st_bufp_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_retry_cnt++;
}

static inline void dpm_reset_st_bufp_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_retry_cnt = 0;
}

static inline unsigned char dpm_get_st_bufp_probe_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_probe_cnt;
}

static inline void dpm_inc_st_bufp_probe_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_cnt++;
}

static inline void dpm_reset_st_bufp_probe_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_cnt = 0;
}

static inline unsigned char dpm_get_st_bufp_arp_cnt(struct dpm_param *dpmp)
{
	return dpmp->st.bufp_arp_cnt;
}

static inline void dpm_inc_st_bufp_arp_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_arp_cnt++;
}

static inline void dpm_reset_st_bufp_arp_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_arp_cnt = 0;
}

static inline unsigned short dpm_get_st_bufp_arpreq_retry_cnt(
														struct dpm_param *dpmp)
{
	return dpmp->st.bufp_arpreq_retry_cnt;
}

static inline void dpm_inc_st_bufp_arpreq_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_arpreq_retry_cnt++;
}

static inline void dpm_reset_st_bufp_arpreq_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_arpreq_retry_cnt = 0;
}

static inline unsigned short dpm_get_st_bufp_probe_retry_cnt(
														struct dpm_param *dpmp)
{
	return dpmp->st.bufp_probe_retry_cnt;
}

static inline void dpm_inc_st_bufp_probe_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_retry_cnt++;
}

static inline void dpm_reset_st_bufp_probe_retry_cnt(struct dpm_param *dpmp)
{
	dpmp->st.bufp_probe_retry_cnt = 0;
}

/*static inline unsigned long dpm_get_st_txretry_type(struct dpm_param *dpmp)
{
	return dpmp->st.txretry_type;
}

static inline void dpm_set_st_txretry_type(struct dpm_param *dpmp,
		unsigned long txretry_type)
{
	dpmp->st.txretry_type = txretry_type;
}*/

static inline void dpm_reset_st_txcnt(struct dpm_param *dpmp)
{
	// memset((void *) &dpmp->st.txcnt, 0, sizeof(struct txcnt_ty));
	dpmp->st.txcnt.ka = 0;
}

static inline int dpm_get_st_txcnt_ka(struct dpm_param *dpmp)
{
	return dpmp->st.txcnt.ka_a[0] +
			dpmp->st.txcnt.ka_a[1] +
			dpmp->st.txcnt.ka_a[2] +
			dpmp->st.txcnt.ka_a[3];
}

static inline void dpm_inc_st_txcnt_ka(struct dpm_param *dpmp, int idx)
{
	dpmp->st.txcnt.ka_a[idx]++;
}

static inline void dpm_reset_st_txcnt_ka(struct dpm_param *dpmp)
{
	dpmp->st.txcnt.ka = 0;
}

static inline int dpm_get_st_txcnt_setps(struct dpm_param *dpmp)
{
	return dpmp->st.txcnt.setps_a[0] +
			dpmp->st.txcnt.setps_a[1] +
			dpmp->st.txcnt.setps_a[2] +
			dpmp->st.txcnt.setps_a[3];
}

static inline void dpm_inc_st_txcnt_setps(struct dpm_param *dpmp, int idx)
{
	dpmp->st.txcnt.setps_a[idx]++;
}

static inline void dpm_reset_st_txcnt_setps(struct dpm_param *dpmp)
{
	dpmp->st.txcnt.setps = 0;
}
...
static inline long long dpm_get_aptrk_bcn_clk(struct dpm_param *dpmp)
{
	return dpmp->aptrk.bcn_clk;
}

static inline void dpm_set_aptrk_bcn_clk(struct dpm_param *dpmp,
		long long bcn_clk)
{
	dpmp->aptrk.bcn_clk = bcn_clk;
}

static inline long long dpm_get_aptrk_bcn_timestamp(struct dpm_param *dpmp)
{
	return dpmp->aptrk.bcn_timestamp;
}

static inline void dpm_set_aptrk_bcn_timestamp(struct dpm_param *dpmp,
		long long bcn_timestamp)
{
	dpmp->aptrk.bcn_timestamp = bcn_timestamp;
}
...
static inline unsigned char dpm_get_sche_feature(struct dpm_param *dpmp)
{
	return dpmp->sche.feature;
}

static inline void dpm_set_sche_feature(struct dpm_param *dpmp,
		unsigned char feature)
{
	dpmp->sche.feature = feature;
}

static inline unsigned long long dpm_get_sche_last_tim_wakeup_clk(
        struct dpm_param *dpmp)
{
	return (unsigned long long) dpmp->sche.last_tim_wakeup_clk;
}

static inline void dpm_set_sche_last_tim_wakeup_clk(struct dpm_param *dpmp,
		unsigned long long last_tim_wakeup_clk)
{
	dpmp->sche.last_tim_wakeup_clk = (long long) last_tim_wakeup_clk;
}
...
static inline unsigned long dpm_get_sche_interval(
		struct dpm_param *dpmp, int n)
{
	return (unsigned long) dpmp->sche.node[n].interval;
}
...
{
	dpmp->sche.node[n].arbitrary = (unsigned int) (arbitrary & 0x3ffffff);
}
...
{
	dpmp->sche.node[n].align = (unsigned char) (align & 0x1);
}
...
{
	dpmp->sche.node[n].half = (unsigned char) (half & 0x1);
}
...
{
	dpmp->sche.node[n].preparation = (unsigned char) (preparation & 0xf);
}
...
#else
static inline unsigned short dpm_get_dbg_module(struct dpm_param *dpmp)
{
	return dpmp->phy.dbg_module;
}

static inline void dpm_set_dbg_module(struct dpm_param *dpmp, unsigned short module)
{
	dpmp->phy.dbg_module = module;
}

static inline unsigned char dpm_get_dbg_severity(struct dpm_param *dpmp)
{
	return dpmp->phy.dbg_severity;
}

static inline void dpm_set_dbg_severity(struct dpm_param *dpmp, unsigned char severity)
{
	dpmp->phy.dbg_severity = severity;
}

static inline unsigned char dpm_get_dbg_print(struct dpm_param *dpmp)
{
    return dpmp->phy.dbg_print;
}

static inline void dpm_set_dbg_print(struct dpm_param *dpmp, unsigned char print)
{
	dpmp->phy.dbg_print = print;
}
#endif

static inline unsigned short dpm_get_phy_vco_lock(struct dpm_param *dpmp)
{
	return dpmp->phy.vco_lock;
}

static inline void dpm_set_phy_vco_lock(struct dpm_param *dpmp,
		unsigned short vco_lock)
{
	dpmp->phy.vco_lock = vco_lock;
}
...
static inline unsigned long dpm_get_phy_otp_cal_reg3(struct dpm_param *dpmp)
{
	return dpmp->phy.otp_cal_reg3;
}

static inline void dpm_set_phy_otp_cal_reg3(struct dpm_param *dpmp,
		unsigned long reg3)
{
	dpmp->phy.otp_cal_reg3 = reg3;
}
