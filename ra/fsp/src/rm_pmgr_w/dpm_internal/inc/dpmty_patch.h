...
#ifndef __DPMTY_PATCH_H__
#define __DPMTY_PATCH_H__
...
#define RST_FEATURE(n)          (dpm_set_env_feature(dpmp, dpm_get_env_feature(dpmp) & (~(n))))
...
#define RST_TX_FEATURE(n)       (dpm_set_env_tx_feature(dpmp, dpm_get_env_tx_feature(dpmp) & (~(n))))
...
#define RST_ST_STATUS(n)        { unsigned long st = dpm_get_st_status(dpmp); dpm_reset_st_status(dpmp); st &= (~(n)); dpm_set_st_status(dpmp, st); }
...
#define RST_DBG_FEATURE(n)      (dpm_set_env_dbg_feature(dpmp, dpm_get_env_dbg_feature(dpmp) & (~(n))))
...
#define RST_FILTER(n)           (dpm_set_env_rx_filter(dpmp, dpm_get_env_rx_filter(dpmp) & (~(n))))
...
#define RST_IP_FILTER(n)        (dpm_set_env_ip_filter(dpmp, dpm_get_env_ip_filter(dpmp) & (~(n))))
...
#define RST_TIMD_FEATURE(n)     (dpm_set_env_timd_feature(dpmp, dpm_get_env_timd_feature(dpmp) & (~(n))))
#define CHK_NO_SCHE_FEATURE(n)  (!(dpm_get_sche_feature(dpmp) & (n)))
#define CHK_SCHE_FEATURE(n)     (dpm_get_sche_feature(dpmp) & (n))
#define MK_SCHE_FEATURE(n)      (dpm_set_sche_feature(dpmp, dpm_get_sche_feature(dpmp) | (n)))
#define RST_SCHE_FEATURE(n)     (dpm_set_sche_feature(dpmp, dpm_get_sche_feature(dpmp) & (~(n))))
...
//#define RST_TIM_FEATURE(n)      (dpm_set_env_tim_feature(dpmp, dpm_get_env_tim_feature(dpmp) & (~(n))))
...
//#define RST_TIMP_FEATURE(n)     (dpm_set_env_timp_feature(dpmp, dpm_get_env_timp_feature(dpmp) & (~(n))))
...
#define TU2MS(tu)    ((((long long) (tu)) << 10LL) / 1000LL)

#define TU2US(tu)    (((long long) (tu)) << 10LL) /* 1 Tu = 1024 us */
#define US2TU(us)    (((long long) (us)) >> 10LL) /* 1 Tu = 1024 us */
#define US2MS(us)    (((long long) (us)) / 1000LL)
#define US2SEC(us)   (((long long) (us)) / 1000000LL)
#define MS2US(ms)    (((long long) (ms)) * 1000LL)
#define SEC2MS(sec)  (((long long) (sec)) * 1000LL)
#define SEC2US(sec)  (((long long) (sec)) * 1000000LL)
 ...
 * @details MSB 8 bits is STE mask and LSB 24 bits is ST mask.
 */
enum {
	...
	DPM_ST_NO_UC                    = 0x00080000,

	/// BUFP is complete or changed.
	DPM_ST_BUFP_DONE                = 0x00100000,
...
enum DPM_FEATURE {
	...
	DPM_FEATURE_ACTIVE_CONTROL     = 0x00000010,
	DPM_FEATURE_SET_PSPOLL         = 0x00000020,
...
enum DPM_DBG_FEATURE {
	...
	// DBG Feature
	DPM_DBG_WAIT_FOR_DPMRESUME = 0x00800000,
	DPM_DBG_FORCED_SLEEP       = 0x01000000,
	DPM_DBG_FORCED_FULLBOOT    = 0x02000000,
	/*DPM_DBG_TIM_TEST_MODE      = 0x00200000,*/ /* Deprecated */
	/// It doesn't use PBR function.
	DPM_DBG_PBR_OFF            = 0x04000000,
...
enum DPM_TRG_FBOOT {
	DPM_TRG_FBOOT_PREPROCESS   = 0x70000000,
	DPM_TRG_FBOOT_LAUNCHER     = 0x80000000,
	DPM_TRG_FBOOT_LMAC         = 0x90000000,
	DPM_TRG_FBOOT_LMAC_LOOP    = 0x91000000,
	DPM_TRG_FBOOT_ABNORMAL     = 0xa0000000
};

enum DPM_TRG_RA {
	DPM_TRG_RA_SCHEDULER       = 0x70000000,
	DPM_TRG_RA_SCHEDULER_END   = 0x71000000,
	DPM_TRG_RA_CALLBACK        = 0x80000000,
	DPM_TRG_RA_CALLBACK_END    = 0x81000000
};
...
enum DPM_TRG_PTIM {
	...
	DPM_TRG_TIM_DOZE_END       = 0xb1000000,
	DPM_TRG_TIM_LOG            = 0xc0000000,
	DPM_TRG_TIM_POWERDOWN      = 0xd0000000,
	DPM_TRG_TIM_FULLBOOT       = 0xe0000000,
	DPM_TRG_TIM_FULLBOOT_OTP   = 0xe1000000,
	DPM_TRG_TIM_FULLBOOT_PAD   = 0xe3000000,
	DPM_TRG_TIM_FULLBOOT_BOOT  = 0xe4000000,
	DPM_TRG_TIM_FULLBOOT_E     = 0xee000000 /* Fullboot Error */
};
...
enum DPM_SCHE_FEATURE {
    ...
    /// PTIM can restart the PTIM's function if necessary.
    DPMSCHE_F_RESTART      = 0x02,

    /// Fullboot doesn't reschedule PTIM functions.
    DPMSCHE_F_KEEP         = 0x04
};
...
/**
 * enum   DPM RX Status
 * @brief  RX Status for UC check
 */
enum DPM_RX_STATUS {
	DPM_RX_STATUS_ARPREQ      = 0x00000001
};

enum DPM_BUFP_STATUS {
	DPM_BUFP_READY,
	DPM_BUFP_ARPREQ,
	DPM_BUFP_PROBE,
	DPM_BUFP_UCHK,
	/// The AP can keep the UC signal during the BUFP check period and transmits
	/// a buffered ARP REPLY frame to the station.
	DPM_BUFP_SUCCESS,
	/// The AP can't keep the UC signal during the BUFP check period.
	DPM_BUFP_FAILURE,
	/// There was an error when running BUFP., It usually means timeout.
	DPM_BUFP_ERROR
};
#endif /*CFG_WIFI*/
#endif //__DPMTY_PATCH_H__
